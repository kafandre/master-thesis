...
class TreeBaseLearner:
    """Tree base learner - degrees of freedom controlled through max_depth."""
    
    def __init__(self, max_depth: int = 3, random_state: Optional[int] = None):
...
class SplineBaseLearner:
    """B-spline base learner with regularization for fair selection."""
    
    def __init__(self, n_knots: int = 5, degree: int = 3, degrees_of_freedom: int = 2):
    ...
    def _create_bspline_basis(self, X: np.ndarray) -> np.ndarray:
        """Create B-spline basis matrix."""
        X_flat = X.flatten()
        x_min, x_max = X_flat.min(), X_flat.max()
        
        # Create knot vector
        if self.n_knots <= self.degree + 1:
            # Minimum knots for given degree
            interior_knots = np.linspace(x_min, x_max, max(2, self.n_knots - self.degree + 1))[1:-1]
        else:
            interior_knots = np.linspace(x_min, x_max, self.n_knots - self.degree + 1)[1:-1]
        
        # Full knot vector with boundary knots repeated
        ...
        basis_matrix = np.zeros((len(X_flat), n_basis))
        
        for i in range(n_basis):
            # Create B-spline basis function
            for j, x_val in enumerate(X_flat):
                basis_matrix[j, i] = self._bspline_basis(x_val, i, self.degree)
        
        return basis_matrix
    
    def _bspline_basis(self, x: float, i: int, k: int) -> float:
        """Evaluate B-spline basis function using de Boor's algorithm."""
        if k == 0:
            return 1.0 if self.knots[i] <= x < self.knots[i + 1] else 0.0
        
        # Handle edge case for last knot
        ...
        if denom_left != 0:
            left_term = (x - self.knots[i]) / denom_left * self._bspline_basis(x, i, k - 1)
        
        denom_right = self.knots[i + k + 1] - self.knots[i + 1]
        if denom_right != 0:
            right_term = (self.knots[i + k + 1] - x) / denom_right * self._bspline_basis(x, i + 1, k - 1)
        
        return left_term + right_term
    
    def fit(self, X: torch.Tensor, y: torch.Tensor):
        """Fit spline model with regularization."""
        if isinstance(X, torch.Tensor):
            X_np = X.detach().numpy()
        else:
            X_np = X
        ...
        # Create basis matrix
        B = self._create_bspline_basis(X_np)
        ...
        # Second-order difference penalty matrix for smoothness
        if n_params > 2:
            D = np.diff(np.eye(n_params), n=2, axis=0)
            penalty_matrix = D.T @ D
        else:
            penalty_matrix = np.eye(n_params)
        
        # Regularized least squares
        BtB = B.T @ B
        if lambda_reg > 0:
            BtB += lambda_reg * penalty_matrix
        
        try:
            self.coef_ = np.linalg.solve(BtB, B.T @ y_np)
        except np.linalg.LinAlgError:
            self.coef_ = pinv(BtB) @ (B.T @ y_np)
    ...
    def predict(self, X: torch.Tensor) -> np.ndarray:
        ...
        # Create basis matrix for prediction
        B = self._create_bspline_basis(X_np)
        return B @ self.coef_